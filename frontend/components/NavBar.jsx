  ...
  return (
    <div className={`${location.pathname === "/login" || location.pathname === "/register" ? "hidden" : "block"} w-screen bg-blue-50`}>
      ...
      <nav className="bg-indigo-950 h-10">
        ...
        {/* Logout button when user has logged in  */}
        <div
          className={`${username ? "block" : "hidden"} h-full mx-8 lg:mx-auto py-2 lg:px-8 flex flex-row justify-center sm:justify-end  items-center lg:max-w-screen-0.5xl gap-8`}>
        ...
        {/* signin/register page redirect when user is logged out */}
        <div
          className={`${!username ? "block" : "hidden"} h-full mx-8 lg:mx-auto py-2 lg:px-8 flex flex-row justify-center sm:justify-end  items-center lg:max-w-screen-0.5xl gap-8`}>
          <button
            onClick={signin}
            className="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 h-full text-center">Signin/Register</button>
